import { Company, EventRecord, HoldingState, Manager, ManagerBonus, Sector, UpgradeDefinition, UpgradeEffect } from './holdingTypes';

const STORAGE_KEY = 'factory-holding-tycoon-v1';

const COMPANY_NAMES = ['Marcenaria Popular', 'Vidraçaria Industrial', 'Oficina de Metal', 'Fábrica de Móveis Planejados', 'Distribuidora Regional', 'Padaria Industrial', 'Confecção Têxtil', 'Metalúrgica Pequena', 'Fábrica de Embalagens', 'Indústria de Esquadrias'];
...
const MANAGER_TEMPLATES: Array<Omit<Manager, 'id' | 'assignedCompanyId' | 'hired'>> = [
...
export function calculateValuation(company: Company): number {
  ...
  if (profit > 2500) return Math.max(0, Math.round(profit * company.valuationMultiple + assetBase - company.debt - riskPenalty));
...
function makeCompany(index = 0, rare = false): Company {
  ...
  const monthlyCost = Math.round(monthlyRevenue * clamp(rand(0.72, 1.06) - efficiency / 420, 0.55, 1.12));
  const debt = Math.round(monthlyRevenue * rand(0.05, 0.55));
  const valuationMultiple = Math.round(rand(10, 24) + quality / 18 + potential / 20);
  ...
  recalc(company);
  company.purchasePrice = Math.max(8000, Math.round(company.estimatedValue * (rare ? 0.62 : rand(0.72, 1.05))));
  return company;
}

function createMarket(count = 6): Company[] { return Array.from({ length: count }, (_, index) => makeCompany(index, Math.random() < 0.16)); }
function createManagers(): Manager[] { return MANAGER_TEMPLATES.map((manager) => ({ ...manager, id: id('manager'), hired: false, assignedCompanyId: null })); }

export function freshHolding(): HoldingState {
  const state: HoldingState = { capital: 85000, month: 1, reputation: 1, activeTab: 'dashboard', market: createMarket(6), portfolio: [], managers: createManagers(), history: [] };
...
export function loadHolding(): HoldingState {
  try {
    ...
    parsed.managers = parsed.managers.map((manager) => ({ ...manager, hired: Boolean(manager.hired) }));
    return parsed;
  } catch { return freshHolding(); }
}

export function saveHolding(state: HoldingState): void { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); }
export function resetHolding(): HoldingState { localStorage.removeItem(STORAGE_KEY); return freshHolding(); }
...
export function getMonthlyProfit(state: HoldingState): number {
  return state.portfolio.reduce((sum, company) => sum + company.monthlyProfit, 0) - state.managers.reduce((sum, manager) => sum + (manager.hired && manager.assignedCompanyId ? manager.salaryMonthly : 0), 0);
}
export function getTotalDebt(state: HoldingState): number { return state.portfolio.reduce((sum, company) => sum + company.debt, 0); }
export function getHoldingValue(state: HoldingState): number { return state.capital + state.portfolio.reduce((sum, company) => sum + company.estimatedValue, 0); }
...
export function buyCompany(state: HoldingState, companyId: string): void {
  ...
  if (state.capital < company.purchasePrice) { addHistory(state, 'Capital insuficiente', `${company.name} custa ${money(company.purchasePrice)}.`, 'bad'); return; }
...
export function sellCompany(state: HoldingState, companyId: string): void {
  ...
  if (company.managerId) { const manager = state.managers.find((item) => item.id === company.managerId); if (manager) manager.assignedCompanyId = null; }
...
export function applyUpgrade(state: HoldingState, companyId: string, upgradeId: string): void {
  ...
  if (!company || !upgrade || company.upgradesApplied.includes(upgrade.id)) return;
  if (state.capital < upgrade.cost) { addHistory(state, 'Capital insuficiente', `Upgrade ${upgrade.name} custa ${money(upgrade.cost)}.`, 'bad'); return; }
...
export function hireManager(state: HoldingState, managerId: string): void {
  ...
  if (!manager) return;
  if (manager.hired) { addHistory(state, 'Manager já contratado', `${manager.name} já faz parte da holding.`, 'neutral'); return; }
  const signingCost = manager.salaryMonthly * 2;
  if (state.capital < signingCost) { addHistory(state, 'Capital insuficiente', `Contratar ${manager.name} exige ${money(signingCost)}.`, 'bad'); return; }
...
export function assignManager(state: HoldingState, managerId: string, companyId: string): void {
  ...
  if (!manager.hired) { addHistory(state, 'Manager não contratado', `Contrate ${manager.name} antes de atribuir.`, 'bad'); return; }
  if (manager.assignedCompanyId && manager.assignedCompanyId !== companyId) { addHistory(state, 'Manager ocupado', `${manager.name} já está atribuído a outra empresa.`, 'bad'); return; }
  if (company.managerId && company.managerId !== managerId) { const old = state.managers.find((item) => item.id === company.managerId); if (old) old.assignedCompanyId = null; }
...
function managerEffect(company: Company, manager: Manager): void {
  ...
  if (manager.type === 'Turnaround' && company.monthlyProfit < 0) { company.monthlyCost = Math.round(company.monthlyCost * 0.94); company.risk -= 4; }
  if (manager.type === 'Escala' && company.monthlyProfit > 8000) { company.monthlyRevenue = Math.round(company.monthlyRevenue * 1.04); company.potential += 2; }
...
export function passMonth(state: HoldingState): void {
  ...
  for (const company of state.portfolio) {
    const manager = company.managerId ? state.managers.find((item) => item.id === company.managerId && item.hired) : null;
    if (manager) managerEffect(company, manager);
    const riskCost = company.risk > 70 && Math.random() < 0.2 ? company.monthlyRevenue * 0.06 : 0;
    company.monthlyRevenue = Math.round(company.monthlyRevenue * rand(0.94, 1.07) * (1 + (company.demand - 50) / 900));
    company.monthlyCost = Math.round(company.monthlyCost * rand(0.98, 1.04) + riskCost);
    recalc(company);
    net += company.monthlyProfit;
  }
  const salaries = state.managers.reduce((sum, manager) => sum + (manager.hired && manager.assignedCompanyId ? manager.salaryMonthly : 0), 0);
  net -= salaries;
  state.capital += net;
...
function triggerRandomEvent(state: HoldingState): void {
  ...
  if (!target || roll < 0.14) { const company = makeCompany(state.month, true); state.market.unshift(company); addHistory(state, 'Oportunidade rara', `${company.name} apareceu barata no mercado.`, 'good'); return; }
  if (roll < 0.28) { target.demand -= 8; target.monthlyRevenue = Math.round(target.monthlyRevenue * 0.92); addHistory(state, 'Crise no setor', `${target.sector} sofreu queda de demanda. ${target.name} perdeu receita.`, 'bad'); }
  else if (roll < 0.42) { target.monthlyRevenue = Math.round(target.monthlyRevenue * 1.12); target.demand += 6; addHistory(state, 'Boom econômico', `${target.name} vendeu mais com o mercado aquecido.`, 'good'); }
  else if (roll < 0.56) { target.efficiency -= 8; target.monthlyCost = Math.round(target.monthlyCost * 1.08); addHistory(state, 'Máquina quebrou', `${target.name} perdeu eficiência e teve aumento de custos.`, 'bad'); }
  else if (roll < 0.7) { target.monthlyCost = Math.round(target.monthlyCost * 1.08); addHistory(state, 'Fornecedor aumentou preço', `${target.name} teve pressão nos custos.`, 'bad'); }
  else if (roll < 0.84) { target.demand += 10; target.monthlyRevenue = Math.round(target.monthlyRevenue * 1.1); addHistory(state, 'Produto viralizou', `${target.name} ganhou demanda repentina.`, 'good'); }
  else { const fine = target.risk > 55 ? Math.round(target.risk * 220) : 0; state.capital -= fine; addHistory(state, 'Fiscalização', fine ? `${target.name} pagou multa de ${money(fine)} por risco alto.` : `${target.name} passou pela fiscalização sem multa.`, fine ? 'bad' : 'neutral'); }
  recalc(target);
}
